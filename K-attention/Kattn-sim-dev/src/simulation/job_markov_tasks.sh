...
# NOTE: --output is set dynamically by submit_markov_tasks.sh to logs/{task_name}/%j.out
...
echo "[markov-tasks] model=$MODEL config=$TEST_CONFIG n=$SAMPLE_SIZE seed=$SEED lr=$LR start=$(date)"
python run_bmk.py \
    --model-type      "$MODEL" \
    ...
    --version         "$SEED"
echo "[markov-tasks] model=$MODEL config=$TEST_CONFIG n=$SAMPLE_SIZE seed=$SEED done=$(date)"
