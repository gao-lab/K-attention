...
echo "============================================"
echo "[rc-tasks] model=$MODEL config=$TEST_CONFIG n=$SAMPLE_SIZE seed=$SEED lr=$LR batch=$BATCH"
echo "[rc-tasks] start=$(date -Iseconds)"
echo "[rc-tasks] host=$(hostname)"
echo "============================================"
...
RC=$?
if [ $RC -eq 0 ]; then
    echo "[rc-tasks] SUCCESS model=$MODEL config=$TEST_CONFIG n=$SAMPLE_SIZE seed=$SEED done=$(date -Iseconds)"
else
    echo "[rc-tasks] FAILED model=$MODEL config=$TEST_CONFIG n=$SAMPLE_SIZE seed=$SEED exit=$RC time=$(date -Iseconds)"
fi
exit $RC
