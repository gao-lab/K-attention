#!/bin/bash
# CRISPR CNN-Transformer baseline — 5-fold CV
# 11 datasets × 2 models (cnn_transformer, cnn_transformer_pm) × 5 sets × version=0
# = 110 runs total (~15-20 GPU-hours on RTX 4090)
# 已完成的 run 自动跳过（基于 test_metrics.csv 存在判断）
...
for DS in "${DATASETS[@]}"; do
    ...
    for SET in "${SETS[@]}"; do
        # 预缓存（只需一次；若 cache 已存在自动跳过）
        CACHE_FLAG=$HF_DATASETS_CACHE/cached_${DS}_${SET}
