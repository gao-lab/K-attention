...
class KattentionV7(nn.Module):
    ...
    def forward(
        ...
        r"""
        X: (B, L_orig, C)  C == channel_size
        return:
          attn_probs:  (B, H, L, 3)  3> [i-1, i, i+1]
          attn_logits: (B, H, L, 3)
          Q_W:         (B, L, H, D = kernel_size * C)
          K:           (B, L, D)
         L = L_orig - kernel_size + 1
        """
        B, L_orig, C = X.shape
        assert C == self.channel_size, "channel_size mismatch"

        X_flat = rearrange(X, "b l c -> b (l c)")

        if self.reverse:
            X_rev = X.flip([1])
            X_rev_flat = rearrange(X_rev, "b l c -> b (l c)")

        # Q: (B, L, k*C), L = L_orig - kernel_size + 1
        Q = X_flat.unfold(
            dimension=1,
            size=self.kernel_size * self.channel_size,
            step=self.channel_size,
        )  # (B, L, k*C)
        ...
        # ----i-1, i, i+1 ----
        # QW: (B, H, L, D)
        QW = rearrange(Q_W, "b l h d -> b h l d")

        # center i * K[i] : (B, H, L)
        center = torch.einsum("bhld,bld->bhl", QW, K)

        left = center.new_zeros(B, self.num_kernels, L)
        right = center.new_zeros(B, self.num_kernels, L)
        ...
        #  (B, H, L, 3): [i-1, i, i+1]
        attn_logits = torch.stack([left, center, right], dim=-1)
        ...
        # ---- padding
        NEG_INF = -1e4
        attn_logits[:, :, 0, 0] = NEG_INF
        attn_logits[:, :, -1, 2] = NEG_INF
        ...
        return {
            "attn_probs": attn_probs,    # (B, H, L, 3)
            "attn_logits": attn_logits,  # (B, H, L, 3)
            "Q_W": Q_W,                  # (B, L, H, D)
            "K": K,                      # (B, L, D)
        }

class KattentionV8(nn.Module):
    def __init__(
        self,
        channel_size: int,
        kernel_size: int = 10,
        ...
        # depthwise 1D conv：按 kernel_size 分组，相当于每个相对位置一个独立线性映射
        self.Wattn = nn.Conv1d(
            in_channels=kernel_size * channel_size,
            out_channels=kernel_size * channel_size * num_kernels,
            kernel_size=1,
    ...
    def forward(
        self,
        X: torch.Tensor,
        key_padding_mask: Optional[torch.Tensor] = None,  # 暂时不用，只占位
    ):
        r"""
        X: (B, L_orig, C)  其中 C == channel_size

        返回:
          attn_probs:  (B, H, L, 3)  最后一维为 [i-1, i, i+1]
          attn_logits: (B, H, L, 3)
          Q_W:         (B, L, H, D)  D = kernel_size * C
          K:           (B, L, D)
        其中 L = L_orig - kernel_size + 1 （patch 序列长度）
        """
        B, L_orig, C = X.shape
        assert C == self.channel_size, "channel_size mismatch"

        # ---- 展平到一维时间轴，便于 unfold ----
        # 这里用 view 而不是 rearrange，稍微少一层函数
        X_flat = X.view(B, L_orig * C)

        if self.reverse:
            X_rev = X.flip(1)
            X_rev_flat = X_rev.view(B, L_orig * C)

        # ---- 构造 Q / K patch 序列 ----
        # 每个 patch 覆盖 kernel_size 个 token：size = k*C, step = C
        # patches: (B, L, k*C)，L = L_orig - k + 1
        patches = X_flat.unfold(
            dimension=1,
            size=self.kernel_size * self.channel_size,
            step=self.channel_size,
        ).contiguous()  # contiguous 有助于后续 Conv / einsum

        if self.reverse:
            K_patches = X_rev_flat.unfold(
                dimension=1,
                size=self.kernel_size * self.channel_size,
                step=self.channel_size,
            ).contiguous()
        else:
            K_patches = patches  # 共享内存视图

        # ---- 用 depthwise conv 生成多 head 的 Q ----
        # Q_conv: (B, k*C, L)
        Q_conv = patches.transpose(1, 2).contiguous()
        # Q_W_conv: (B, k*C*H, L)
        Q_W_conv = self.Wattn(Q_conv)

        # 变成 (B, H, L, D) 直接适配 einsum，避免后面多次 transpose
        # D = k*C
        Q_W = rearrange(
            Q_W_conv,
            "b (k h c) l -> b h l (k c)",
        ...
        assert B2 == B
        # K: (B, L, D)
        K = K_patches.view(B, L, D).contiguous()

        # 做完这些大张量后，可以把不再用的中间变量删掉，便于显存复用
        del Q_conv, Q_W_conv, patches, K_patches

        # ---- 分配最终 logits：只保留一个 (B,H,L,3) 大张量 ----
        attn_logits = X.new_empty(B, H, L, 3)

        # center: i 对 K[i]，形状 (B,H,L)
        attn_logits_center = torch.einsum(
            "bhld,bld->bhl",
            Q_W,     # (B,H,L,D)
            K,       # (B,L,D)
        )
        attn_logits[:, :, :, 1] = attn_logits_center

        if L > 1:
            # left: i 对 K[i-1]，只在 i>=1 有效
            attn_logits_left = torch.einsum(
                "bhld,bld->bhl",
                Q_W[:, :, 1:, :],   # (B,H,L-1,D)
                K[:, :-1, :],       # (B,L-1,D)
            )
            # 填到 i>=1 的位置，i=0 留给后面设 -inf
            attn_logits[:, :, 1:, 0] = attn_logits_left

            # right: i 对 K[i+1]，只在 i<=L-2 有效
            attn_logits_right = torch.einsum(
                "bhld,bld->bhl",
                Q_W[:, :, :-1, :],  # (B,H,L-1,D)
                K[:, 1:, :],        # (B,L-1,D)
            )
            attn_logits[:, :, :-1, 2] = attn_logits_right
        else:
            # L==1 的极端情况：直接把两侧设为 -inf
            attn_logits[:, :, 0, 0] = float("-inf")
            attn_logits[:, :, 0, 2] = float("-inf")
        ...
        # ---- softmax + dropout ----
        attn_probs = F.softmax(attn_logits, dim=-1)  # 最后一维(3)归一化
        attn_probs = self.attn_drop(attn_probs)

        # 为了和之前接口保持一致，把 Q_W 变回 (B,L,H,D)
        Q_W_out = rearrange(Q_W, "b h l d -> b l h d")
...
# class MarkovConv1d(nn.Module):
#     """
#     一阶马尔可夫“卷积”：
#     对每个 fragment X[b, i:i+l, :] 计算
#         y[b, i, h] = sum_{t=0}^{l-2} x[b, i+t]^T T[h] x[b, i+t+1]
#     其中 T[h] 是第 h 个马尔可夫核的转移矩阵 (C x C)
#     """

#     def __init__(self, in_channels: int, num_kernels: int, kernel_size: int):
#         super().__init__()
#         assert kernel_size >= 2, "Markov kernel_size 至少要 2，才有相邻对"
...
#             torch.randn(num_kernels, in_channels, in_channels)
#         )

#         # 第二步：对 pair score 做一个 [1,1,...,1] 的卷积来做窗口内求和
#         # 输入 S 形状: (B, H, L_pairs)
#         # 输出 Y: (B, H, L), L = L_orig - kernel_size + 1
#         self.window_agg = nn.Conv1d(
#             in_channels=num_kernels,
#             out_channels=num_kernels,
#             kernel_size=kernel_size - 1,  # 因为 pair 的长度是 L_orig - 1
#             groups=num_kernels,
#             bias=False,
#         )

#         # 初始化把卷积核固定为 1，相当于简单求和
#         nn.init.ones_(self.window_agg.weight)
#         # 如果你希望“窗口内的 pair 加权求和”，可以把这个参数保留为可训练
...
#         # 2. 计算所有相邻 pair 的 Markov score
#         # transition: (H, C, C)
#         # einsum: 'btc,hcd,btd->bht'
#         # 解释：
#         #   b: batch
#         #   t: pair index (0..L_orig-2)
#         #   c,d: channel
#         S = torch.einsum("btc,hcd,btd->bht", X_L, self.transition, X_R)
#         # S: (B, H, L_pairs), L_pairs = L_orig - 1

#         # 3. 在 pair 维度上做长度 (kernel_size-1) 的卷积求和
#         # 先把 S 调整为 Conv1d 的格式 (B, H, L_pairs)
#         Y = self.window_agg(S)  # (B, H, L)

#         # 4. 调整为 (B, L, H)，和常规 Conv1d 输出保持一致风格
...
class MarkovConv1d(nn.Module):
    ...
    位置特异的一阶马尔可夫“卷积”：
    对每个 fragment X[b, i:i+l, :] 计算
        y[b, i, h] = sum_{t=0}^{l-2} x[b, i+t]^T T[h, t] x[b, i+t+1]
    其中 T[h, t] 是第 h 个核在 fragment 内第 t→t+1 的转移矩阵 (C x C)
    """
    ...
    def __init__(self, in_channels: int, num_kernels: int, kernel_size: int):
        ...
        # T: (H, l-1, C, C)
        self.transition = nn.Parameter(
            torch.randn(num_kernels, kernel_size - 1, in_channels, in_channels) * 0.1
        )
    ...
    def forward(self, X: torch.Tensor) -> torch.Tensor:
        ...
        assert L_orig >= l, f"序列长度 {L_orig} 必须 >= kernel_size {l}"
        ...
        # 2. 在 pair 维度上做长度 (l-1) 的滑动窗口
        # PyTorch unfold: (B, L_pairs, C) -> (B, L_out, C, l-1)
        X_L_win = X_L.unfold(dimension=1, size=l - 1, step=1)  # (B, L_out, C, l-1)
        X_R_win = X_R.unfold(dimension=1, size=l - 1, step=1)  # (B, L_out, C, l-1)

        # 3. 一次性 einsum

        Y = torch.einsum("bict,htcd,bidt->bhi", X_L_win, self.transition, X_R_win)

        return Y

class KattentionV9(nn.Module):
    def __init__(
        self,
        in_channels: int=4,
        num_kernels: int=64,
        ...
        # T: (H, l-1, C, C)
        self.transition = nn.Parameter(
            torch.randn(num_kernels, kernel_size, in_channels, in_channels) * 0.1
        )
    ...
    def forward(self, X: torch.Tensor):
        ...
        返回:
          attn_logits: (B, H, L_out, L_out)
          attn_probs:  (B, H, L_out, L_out)
        ...
        assert L >= l, f"序列长度 {L} 必须 >= kernel_size {l}"

        # 1. unfold 得到 Q 窗口: (B, L_out, l-1, C)
        Q = X.unfold(dimension=1, size=l, step=1)  # (B, L_out, l, C)
        B2, L_out, C2, _ = Q.shape
        assert B2 == B and C2 == C

        # 2. 构造 K
        if self.reverse:
            # 先对时间维翻转，再做相同的 unfold
            X_rev = torch.flip(X, dims=[1])                  # (B, L, C)
            K = X_rev.unfold(dimension=1, size=l, step=1)  # (B, L_out, l, C)
        else:
            # 正常自注意力：K 和 Q 来自同一个方向
            K = Q
        ...
        # 5. softmax 得到注意力权重
        attn_probs = F.softmax(attn_logits, dim=-1)  # 在 key 维 L_out 上归一化

        return {
            "attn_logits": attn_logits,  # (B, H, L_out, L_out)
            "attn_probs": attn_probs,    # (B, H, L_out, L_out)
        }

class KattentionModel_markov(nn.Module):
    def __init__(
        self,
        embedding_method: Literal["onehot", "learned"] = "onehot",
        kattn_version: str = "v4",
        ...
        # self.kattn = KattentionV8(
        #         channel_size=input_embedding_dim,
        #         kernel_size=kernel_size,
        #         num_kernels=num_kernels
        #     )
        self.kattn = KattentionV9(
                in_channels=input_embedding_dim,
                kernel_size=kernel_size,
                num_kernels=num_kernels
            )
    ...
    def forward(
        ...
        attn_logits = self.kattn(X)["attn_logits"]
        pooled_attn = self.max_pool(attn_logits).squeeze(-1).squeeze(-1)
