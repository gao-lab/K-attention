...
def init_parser():
    ...
    parser.add_argument('--entropy', dest='entropy', default=1.0,type=float,
                        help="target entropy [0-2]")
    parser.add_argument('--num', dest='num', default=5000,type=int,
...
def generate_randomized_transition_matrix(states=['A', 'C', 'G', 'T'],rs=12):
    ...
    for prev1 in states:
        for prev2 in states:
            ...
            # 随机参数配置
            k = random.choices([1, 2], weights=[0.8, 0.2])[0]  # 主导状态数量
            dominant_prob = np.random.uniform(0.7, 0.9)  # 主导状态总概率
...
def generate_random_matrix(target_entropy) -> Dict[str, Dict[str, float]]:
    """生成一个4x4的随机概率矩阵，每行和为1"""
    bases = ['A', 'C', 'G', 'T']
    matrix = {}
    if target_entropy>=1:
        for base in bases:
            probs = np.random.dirichlet([1.0, 1.0, 1.0, 1.0])
            # # 随机生成四个0到1之间的随机数
            # random_numbers = np.random.rand(4)
            # 归一化
            # probs = random_numbers / np.sum(random_numbers)
            matrix[base] = {'A': probs[0], 'C': probs[1], 'G': probs[2], 'T': probs[3]}
    elif target_entropy>0.5:
        for base in bases:
...
def calculate_average_entropy(matrix: Dict[str, Dict[str, float]]) -> float:
    """计算平均信息熵"""
    total_entropy = 0.0
    ...
    return total_entropy / 4.0


def format_matrix(matrix: Dict[str, Dict[str, float]], decimals: int = 4) -> Dict[str, Dict[str, float]]:
    """格式化矩阵为指定小数位数，并确保每行和为1"""
    formatted = {}
    for base in matrix:
        row = matrix[base]
        formatted_row = {}
        sorted_nucleotides = sorted(row.keys())  # 确保固定顺序处理

        # 对前三个碱基四舍五入
        sum_first_three = 0.0
        for nt in sorted_nucleotides[:-1]:
            formatted_val = round(row[nt], decimals)
            formatted_row[nt] = formatted_val
            sum_first_three += formatted_val

        # 计算最后一个碱基的值，确保和为1
        last_nt = sorted_nucleotides[-1]
        last_val = round(1.0 - sum_first_three, decimals)
        if last_val < 0:
            last_val = 0.0
            excess = -last_val
            max_nt = max(formatted_row, key=lambda k: formatted_row[k])
            formatted_row[max_nt] = round(formatted_row[max_nt] - excess, decimals)
        formatted_row[last_nt] = last_val

        formatted[base] = formatted_row
    return formatted


def generate_entropy_matrix(
    ...
    :param target_entropy: 目标平均信息熵 (范围0~2)
    :param tolerance: 允许的熵波动范围
    :param max_attempts: 最大尝试次数
    :param decimals: 输出保留小数位数
    :return: 格式化的马尔可夫矩阵
    """
    for _ in range(max_attempts):
        matrix = generate_random_matrix(target_entropy)
        avg_entropy = calculate_average_entropy(matrix)
        if (target_entropy - tolerance) <= avg_entropy <= (target_entropy + tolerance):
            return avg_entropy, format_matrix(matrix, decimals)
    raise RuntimeError(f"在{max_attempts}次尝试内未找到符合条件的矩阵。")
...
def main():
    ...
    seqNum = args.num
    name = str(target_entropy).replace('.', '_')

    # seqNum = 5000
    seqLen = 100
    output_dir = f"/Kattn-sim-dev/resources/markov_{name}_{seqNum}.fa"
    first_p = {'A': 0.25, 'C': 0.25, 'G': 0.25, 'T': 0.25}
    trans_mat = []
    for _ in range(9):
        try:
            avg_entropy,matrix = generate_entropy_matrix(
                target_entropy=target_entropy,
                tolerance=0.1,
                max_attempts=100000,
                decimals=2
            )
            print(f'{_} : {avg_entropy}')
        except RuntimeError as e:
            print(e)
        trans_mat.append(matrix)

    # 保存列表到文件
    with open(f'../trans_mat/{name}_{seqNum}.pkl', 'wb') as f:
    ...
    for num in range(InitMatrix.shape[0]):
        ...
        seq_list = generate_first_order(first_p, trans_mat)
